...
def predict_image(image_path, model):
    ...
    return _predict_from_image_array(img_array, model)


def predict_image_with_preview(image_path, model):
    """Loads an image and returns prediction along with the preview image."""
    print(f"\nPredicting image: {image_path}")
    ...
    img_array = tf.keras.utils.img_to_array(img)
    predicted_class, confidence = _predict_from_image_array(img_array, model)
